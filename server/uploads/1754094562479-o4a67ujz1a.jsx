...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const fetchChatHistory = async () => {
      ...
      if (res.ok) mergeChatMessages(await res.json());
    ...
    s.on("chat:message", (msg) => {
      setChatMessages((prev) =>
        prev.length && prev.at(-1).text === msg.text ? prev : [...prev, msg]
      );
    });
  ...
  const parseCustomDuration = () => {
    ...
    return isNaN(m) || isNaN(s) || m < 0 || s < 0 || s > 59 ? null : m * 60 + s;
  };
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-6 shadow-xl flex flex-col items-center">
            ...
            <div className="flex gap-2 w-full mb-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setCustomSeconds(
                    Math.min(59, parseInt(e.target.value || 0)).toString()
                  )
                }
                className="w-16 bg-[#0d1025] border border-yellow-300/20 rounded px-2 py-1 text-sm text-yellow-100"
                placeholder="Sec"
        ...
        <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-6 shadow-xl flex flex-col">
          ...
          <input
            ...
            className="mb-4 block w-full text-sm text-yellow-100 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-orbitron file:bg-gold-200 file:text-black hover:file:brightness-110"
          />
